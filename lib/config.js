...
ConfigDB.prototype.load = function() {
	...
	try {
		...
		this.db.images = require(this.configDir + 'images.json')
	} catch(e) {}
	...
	Object.keys(this.db.images).forEach(function(host) {
		self.db.images[host].forEach(function(image) {
			self.db.index.imageByUUID[image.manifest.uuid] = image
		})
	})
}
...
ConfigDB.prototype.save = function() {
	...
	fs.writeFileSync(this.configDir + 'images.json', JSON.stringify(this.db.images))
}
